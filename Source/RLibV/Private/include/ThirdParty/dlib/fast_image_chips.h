...
// Modifications (original file in comment at end) Copyright Epic Games, Inc. All Rights Reserved.
...
namespace rlibv
{
	class interpolate_nn_with_alpha
	{
	public:
	...
	/**
	 * @brief A function to quickly extract chips from an image and a stack of lower-res version
	 * @tparam image_type1	An image object that implements the interface defined in
							dlib/image_processing/generic_image.h
	 * @tparam image_type1	An image object that implements the interface defined in
							dlib/image_processing/generic_image.h
	 * @param image A const pointer to the full resolution image
	 * @param lower_res_image_stack The lower resolution images, where item 0 is approx 50% smaller than image
	 *								and item[1] is approx 50% smaller than item 1, etc.
	 * @param chip_locations The positions and rotations of the desired chips
	 * @param[out]  The resulting extracted chips
	 * @param[in/out] tp A thread-pool used for the parallelization of this function
	 * @pre chip_locations[i].size() > 0
	 * @pre For all chip locations !chip_locations[i].rect.is_empty()
	 */
	template<typename image_type1,typename image_type2>
	void fast_extract_image_chips(
		const dlib::array2d<image_type1>* image,
		const dlib::array<dlib::array2d<image_type1>>& lower_res_image_stack,
		const std::vector<dlib::chip_details>& chip_locations,
		dlib::array<dlib::array2d<image_type2>>& chips,
		dlib::thread_pool& tp
	)
	{
		...
		int n_chips = static_cast<int>(chips.size());
		std::vector<int> chip_inds = rlibv::linear_range<int>(0, n_chips - 1, n_chips);
		dlib::parallel_for(tp, 0, chip_inds.size(), [&](const long i)
			{
				std::vector<dlib::vector<double, 2> > from, to;
				dlib::set_image_size(chips[i], chip_locations[i].rows, chip_locations[i].cols);

				// now extract the actual chip
				dlib::chip_details this_chip = chip_locations[i];
				int real_width = static_cast<int>((this_chip.rect.tl_corner() - this_chip.rect.tr_corner()).length());
				float width_ratio = static_cast<float>(real_width) / static_cast<float>(this_chip.cols);
				int level= -1;

				if (width_ratio > 1.f)
				{
					level = std::min(static_cast<int>(std::round(std::log2f(width_ratio) - 1.f)), static_cast<int>(lower_res_image_stack.size())-1);
					
				}

				if (level >= 0)
				{
					float r_factor = static_cast<float>(lower_res_image_stack[level].nr()) / static_cast<float>(image->nr());
					float c_factor = static_cast<float>(lower_res_image_stack[level].nc()) / static_cast<float>(image->nc());
					this_chip.rect.left() *= c_factor;
					this_chip.rect.right() *= c_factor;
					this_chip.rect.top() *= r_factor;
					this_chip.rect.bottom() *= r_factor;
				}
				
				// find the appropriate transformation that maps from the chip to the input image
				from.clear();
				to.clear();
				const auto& rect = this_chip.rect;
				from.push_back(get_rect(chips[i]).tl_corner());  to.push_back(dlib::rotate_point<double>(center(rect), rect.tl_corner(), this_chip.angle));
				from.push_back(get_rect(chips[i]).tr_corner());  to.push_back(dlib::rotate_point<double>(center(rect), rect.tr_corner(), this_chip.angle));
				from.push_back(get_rect(chips[i]).bl_corner());  to.push_back(dlib::rotate_point<double>(center(rect), rect.bl_corner(), this_chip.angle));
				dlib::point_transform_affine trns = dlib::find_affine_transform(from, to);				
				if (level == -1)
				{
					dlib::transform_image(*image, chips[i], interpolate_nn_with_alpha(), trns);
				}
				else
				{
					dlib::transform_image(lower_res_image_stack[level], chips[i], interpolate_nn_with_alpha(), trns);
				}
			},1);
	}
...
//// Original code from which these modifications arise
