...
#include <nls/vulkan/ColoredMeshVulkanRenderer.h>

#include <nls/vulkan/shaders/embed_coloredmeshrenderer.vert.spv.h>
#include <nls/vulkan/shaders/embed_coloredmeshrenderer.frag.spv.h>


namespace epic::nls {

struct ColoredMeshVulkanRenderer::UniformBufferObject {
...
ColoredMeshVulkanRenderer::ColoredMeshVulkanRenderer(std::shared_ptr<VulkanDevice> vulkanDevice)
...
ColoredMeshVulkanRenderer::~ColoredMeshVulkanRenderer()
{
    Reset();
}


void ColoredMeshVulkanRenderer::Reset()
{
...
void ColoredMeshVulkanRenderer::Setup(VkRenderPass renderPass, VkSampleCountFlagBits samples)
{
    m_graphicsPipeline = CreateGraphicsPipeline(renderPass, samples);
}


void ColoredMeshVulkanRenderer::UpdateMeshBuffer(VulkanMeshBuffer& meshBuffer, const Eigen::Matrix<float, 4, -1>& colors) const
{
...
void ColoredMeshVulkanRenderer::Render(VkCommandBuffer buffer,
                                ...
                                bool useLighting) const
{
    ...
    VkBuffer buffers[] = {meshBuffer.m_vertexBuffer->Buffer(), meshBuffer.m_normalBuffer->Buffer(), meshBuffer.m_colorBuffer->Buffer()};
    VkDeviceSize offsets[] = {meshBuffer.m_vertexBuffer->Offset(), meshBuffer.m_normalBuffer->Offset(), meshBuffer.m_colorBuffer->Offset()};
    vkCmdBindVertexBuffers(buffer, 0, 3, buffers, offsets);
...
VkPipeline ColoredMeshVulkanRenderer::CreateGraphicsPipeline(VkRenderPass renderPass, VkSampleCountFlagBits samples)
{
    std::unique_ptr<VulkanShaderModule> vertShaderModule = VulkanShaderModule::Create(m_vulkanDevice, SPV_coloredmeshrenderer_vert_spv, sizeof(SPV_coloredmeshrenderer_vert_spv));
    std::unique_ptr<VulkanShaderModule> fragShaderModule = VulkanShaderModule::Create(m_vulkanDevice, SPV_coloredmeshrenderer_frag_spv, sizeof(SPV_coloredmeshrenderer_frag_spv));
    ...
    std::vector<VkVertexInputBindingDescription> bindingDescriptions = {posBindingDescription, normaldBindingDescription, colorBindingDescription};
    ...
    std::vector<VkVertexInputAttributeDescription> attributeDescriptions = {posAttributeDescription, normalAttributeDescription, colorAttributeDescription};
