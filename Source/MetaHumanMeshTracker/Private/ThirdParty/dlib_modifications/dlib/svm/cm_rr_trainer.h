...
// This file adds classe cm_rr_trainer which is a modified (and renamed) version of the original
// rr_trainer class in dlib.
// Class has been placed in a cm namespace.
// The only modification to this class is that it now uses classes cm_decision_function and
// cm_linear_kernel instead of the original corresponding dlib classes.
...
namespace cm
{
    ...
    public:
        ...
        typedef cm_decision_function<kernel_type> trained_function_type;

        // You are getting a compiler error on this line because you supplied a non-linear or 
        // sparse kernel to the cm_rr_trainer object.  You have to use cm::cm_linear_kernel with this trainer.
        COMPILE_TIME_ASSERT((is_same_type<K, cm_linear_kernel<sample_type> >::value));
        ...
        )
        {
            ...
            DLIB_ASSERT(is_vector(lambdas) && lambdas.size() > 0 && min(lambdas) > 0,
                "\t void cm_rr_trainer::set_search_lambdas()"
        ...
        template <
            ...
            >
        const cm_decision_function<kernel_type> train (
    ...
    private:
        ...
        template <
            ...
            typename in_scalar_vector_type
            >
        const cm_decision_function<kernel_type> do_train (
            const in_sample_vector_type& x,
            const in_scalar_vector_type& y,
        ...
        ) const
        {
            ...
            DLIB_ASSERT(is_learning_problem(x,y),
                "\t cm_decision_function cm_rr_trainer::train(x,y)"
            ...
            /*
                ...
                approaches:
                    ...
                    In the implementation of the cm_rr_trainer I'm only using two simple equations
            ...
            */
            ...
            cm_decision_function<kernel_type> df;
            df.alpha.set_size(1);
            df.alpha = 1;
